...
from .commands import ProtocolCommands, RequestCommands  # noqa: DUO125
from .utils import ReplyParser, SpinsolveConnection
...
class SpinsolveNMR:
    ...
    def __init__(
        ...
        self.reference_spectra: dict[str, SpinsolveNMRSpectrum] = {}
        self.reference_fids: dict[str, NDArray] = {}
...
hours ago, please perform CheckShim to check spectrometer performance!"
    ...
    def get_spectrum(self, protocol=None):
        ...
        if self.data_folder_queue.empty():
            ...
            if protocol is None:
                ...
                self.logger.warning(
                    f"Running default {self.DEFAULT_EXPERIMENT[0]} protocol."
                )
    ...
    # High level data processing methods
    def get_processed_data(
        self,
        method: Literal[
            "chemical_conversion", "peak_ratio", "jaccard_index", "monitor_code"
        ],
    ) -> float | None:
        """Returns the processed data from the queue.

        Args:
            method (str): The method to get the processed data for.
        """
        try:
            data_queue: queue.Queue = getattr(self.spectrum.processed_data, method)
            return data_queue.get(block=False)
        except queue.Empty:
            return None

    def calculate_jaccard_to_reference(
        self,
        reference_code: str = "default",
        similarity_reference: Literal["first", "recent"] = "first",
    ) -> float:
        """Calculates Jaccard index with the reference spectrum.

        Args:
            reference_code (str): The code of the reference spectrum to use.
        """
        if reference_code not in self.reference_spectra:
            # when comparing against the first spectrum, the initial comparison always
            # gives 1.0 because the spectrum is compared to itself
            self.reference_spectra[reference_code] = deepcopy(self.spectrum)
            # when comparing against the most recent spectrum, should return nothing
            # because the monitoring algorithm will be confused otherwise (the starting
            # value is unknown in this case)
            if similarity_reference == "recent":
                # returing 0.0 without updating the data queue
                return 0.0

        # calculate Jaccard index with values optimised for NMR spectra
        jaccard_index = jaccard_two_spectra(
            self.reference_spectra[reference_code], self.spectrum, 4, 1000
        )
        self.spectrum.processed_data.jaccard_index.put(jaccard_index)
        # reference needs to be updated only if the similarity reference is 'recent'
        if similarity_reference == "recent":
            self.reference_spectra[reference_code] = deepcopy(self.spectrum)
        return jaccard_index

    def calculate_cross_correlation_to_reference(
        self,
        reference_code: str = "default",
        similarity_reference: Literal["first", "recent"] = "first",
    ) -> float:
        """Calculates cross-correlation with the reference spectrum.

        Args:
            reference_code (str): The code of the reference spectrum to use.
        """
        if reference_code not in self.reference_fids:
            self.reference_fids[reference_code] = self.spectrum.y_data
            if similarity_reference == "recent":
                # returing 0.0 without updating the data queue
                return 0.0

        cross_correlation = cross_correlation_similarity(
            self.reference_fids[reference_code], self.spectrum.y_data
        )
        self.spectrum.processed_data.cross_correlation.put(cross_correlation)
        # reference needs to be updated only if the similarity reference is 'recent'
        if similarity_reference == "recent":
            self.reference_fids[reference_code] = self.spectrum.y_data
