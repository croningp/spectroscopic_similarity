...
from .utils.constants import SAMPLE_TAG, SOLVENT_TAG, USER_DATA_TAG
...
def load_commands_from_file(protocols_path=None):
    ...
    if protocols_path is None:
        ...
        try:
            commands_tree = Et.parse(spinsolve_commands_file)  # nosec B314
        ...
        try:
            commands_tree = Et.parse(protocol_options_file)  # nosec B314
        except ParseError:
...
def load_commands_from_device(device_message):
    ...
    commands_root = Et.fromstring(device_message)  # nosec B314
    protocols = {
        element.get("protocol"): element for element in commands_root.iter("Protocol")
    }
    return protocols
...
class ProtocolCommands:
    ...
    def __iter__(self):
        ...
        yield from self._protocols.keys()
    ...
    def get_protocol(self, protocol_name):
        ...
        try:
            ...
            raise ProtocolError(
                f"Supplied protocol <{protocol_name}> is not a valid protocol"
            ) from None
