...
def jaccard_two_spectra(
    spectrum_1: GenericSpectrum,
    spectrum_2: GenericSpectrum,
    x_precision: int,
    y_error: float,
) -> float:
    """The Jaccard similarity index is a useful metric for comparing two spectra
    when an in-depth analysis of individual peaks is not feasible. The index is
    calculated as the ratio of the intersection area to the union area of the two
    spectra. To obtain these areas, the minimum (/maximum) intensity value of the two
    spectra at each common x-axis point is used to create the intersection (/union)
    spectrum via integration over the common x-axis. To avoid bad conditioning, the
    union and intersection spectra are equalised if the difference in the intensity
    values is smaller than the experimental error y_error.
    """
    common_x_axis = np.round(spectrum_1.x_data, decimals=x_precision)

    # get y-values of second spectrum at common x-axis
    indices = np.digitize(common_x_axis, spectrum_2.x_data, right=True) - 1
    indices = np.clip(indices, 0, len(spectrum_2.y_data) - 1)
    spectrum_2_y = spectrum_2.y_data.real[indices]
    spectrum_1_y = spectrum_1.y_data.real

    # create union and intersection of the spectra
    union, intersection = [], []
    for y1, y2 in zip(spectrum_1_y, spectrum_2_y):
        # equalise union and intersection for small differences to avoid bad conditioning
        if abs(y1 - y2) < y_error:
            union.append(y2)
...
def cross_correlation_similarity(
    ...
    """Calculate the cross-correlation similarity between two spectra. While this is
    technically possible to perform on any data, it is most useful for FIDs or
    spectra with a common x-axis, such as NMR or MS data."""
